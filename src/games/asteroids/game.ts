...
class Bullet {
  x: number; y: number; vx: number; vy: number; ttl: number; radius: number; dead: boolean
...
class Asteroid {
  x: number; y: number; size: number; radius: number; dead: boolean
  vx: number; vy: number; rotSpeed: number; rot: number; verts: [number, number][]
...
class PowerUp {
  x: number; y: number; vx: number; vy: number; radius: number; ttl: number; dead: boolean
...
class Ship {
  ...
  radius: number; thrusting: boolean; invincible: number
  shootCooldown: number; dead: boolean; tripleShot: number

  constructor() {
    this.tripleShot = 0
    this.x = 0; this.y = 0; this.angle = 0; this.vx = 0; this.vy = 0
    this.radius = 0; this.thrusting = false; this.invincible = 0
    this.shootCooldown = 0; this.dead = false
    this.reset()
  }
...
class Particle {
  x: number; y: number; vx: number; vy: number; life: number; ttl: number; dead: boolean
...
function update(dt: number) {
  ...
  if (score !== prevScore) cbs?.onScoreChange(score)
...
export const game: AsteroidsGame = {
  start(cnv: HTMLCanvasElement, callbacks: AsteroidsCallbacks) {
    canvas = cnv
    canvas.width = W
    canvas.height = H
  ...
  destroy() {
    ...
    ctx = null
    cbs = null
    // Clear justPressed to avoid ghost inputs on next start
